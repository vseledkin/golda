...
func (h *HdpLda) Create() {
	// t : table index for document j
	//     t=0 means to draw a new table
	h.using_t = make([][]int, h.M)
	for j := range h.using_t {
		h.using_t[j] = []int{0}
	}
	...
	h.x_ji = docs  # vocabulary for each document and term
	h.k_jt = [numpy.zeros(1, dtype=int) for j in range(self.M)]  // topics of document and table
	h.n_jt = [numpy.zeros(1, dtype=int) for j in range(self.M)]  // number of terms for each table of document
	h.n_jtv = [[None] for j in range(self.M)]

	h.m = 0
	h.m_k = numpy.ones(1, dtype=int)  # number of tables for each topic
	h.n_k = numpy.array([self.beta * self.V])  # number of terms for each topic ( + beta * V )
	h.n_kv = [DefaultDict(0)]  # number of terms for each topic and vocabulary ( + beta )

	//table for each document and term (-1 means not-assigned)
	h.t_ji = [numpy.zeros(len(x_i), dtype=int) - 1 for x_i in docs]
